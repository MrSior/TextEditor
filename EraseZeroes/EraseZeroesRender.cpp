...
EraseZeroesRender::EraseZeroesRender(EraseZeroesModel *model) {
    m_model = model;
    init();
}

void EraseZeroesRender::init() {
    window_x = 946;
    window_y = 110;
...
void EraseZeroesRender::Render() {
    m_window.clear(sf::Color(9,31,47));
    m_window.draw(*this);
    m_window.display();
}

void EraseZeroesRender::draw(sf::RenderTarget &target, sf::RenderStates states) const {
    states.transform *= getTransform();
    sf::RectangleShape cursor(sf::Vector2f(25, 45));
    if (m_model->getCurrentLine() == 0) {
        cursor.setPosition(22 * (m_model->getCurrentCursorPosition() + 14) + 10, 7 + m_model->getCurrentLine() * 45);
    } else {
