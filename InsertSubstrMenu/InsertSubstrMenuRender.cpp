...
InsertSubstrMenuRender::InsertSubstrMenuRender(InsertSubstrMenuModel *model) {
    m_model = model;
    init();
}

void InsertSubstrMenuRender::init() {
    window_x = 946;
    window_y = 155;
...
void InsertSubstrMenuRender::Render() {
    m_window.clear(sf::Color(9,31,47));
    m_window.draw(*this);
    m_window.display();
}

void InsertSubstrMenuRender::draw(sf::RenderTarget &target, sf::RenderStates states) const {
    ...
    std::string line = "Substring: " + m_model->getStringToInsert();
