...
#include "CollapseBracketsController.h"

CollapseBracketsController::CollapseBracketsController(CollapseBracketsModel *model, CollapseBracketsRender *render) {
    m_model = model;
    m_render = render;
}

void CollapseBracketsController::Run() {
    sf::Event event;
    ...
    while (m_render->window().isOpen()){
        while (m_render->window().pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed){
                if (event.key.code == sf::Keyboard::Enter && !m_model->getLineString().empty()){
            ...
            if (event.type == sf::Event::MouseButtonPressed){
                if (event.mouseButton.button == sf::Mouse::Left){
                    int chosen_symbol = (event.mouseButton.x - 6 * 22 - 10) / 22;
