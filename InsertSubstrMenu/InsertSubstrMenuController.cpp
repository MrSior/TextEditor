...
#include "InsertSubstrMenuController.h"

InsertSubstrMenuController::InsertSubstrMenuController(InsertSubstrMenuModel *model, InsertSubstrMenuRender *render) {
    m_model = model;
    m_render = render;
}

void InsertSubstrMenuController::Run() {
    sf::Event event;
            ...
            if (event.type == sf::Event::MouseButtonPressed){
                if (event.mouseButton.button == sf::Mouse::Left){
                    ...
                    if (m_model->getCurrentLine() == 0) {
                        chosen_symbol = (event.mouseButton.x - 11 * 22 - 10) / 22;
